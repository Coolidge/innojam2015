...
void writeToFile(int numberOfFaces);
...
void detectAndDisplay(Mat frame)
{
	...
	if (current_faces != saved_faces)
	{
		saved_faces = current_faces;
		writeToFile(saved_faces);
	}
...
void writeToFile(int numberOfFaces)
{
	if (!myfile.is_open())
	{
		myfile.open("\\\\iltlvp01.tlv.sap.corp\\Public\\ArielA\\eyeLike.txt", ofstream::app);
	}
	myfile << to_string(numberOfFaces) + "\n";
}